...
class CartController extends FrontBaseController
{
    ...
    public function addcart($id)
    {
        ...
        $result = $cart->add($prod, $prod->id,$size,$color,$keys,$values);
        
        // POD CAPACITY ERROR HANDLING
    ...
    public function addtocart($id)
    {
        ...
        if ($result === false && isset($cart->pod_capacity_error)) {
            return redirect()->route('front.cart')->with('unsuccess', 
                __('Production capacity reached for today. Only') . ' ' . $cart->pod_remaining . ' ' . __('units available.')
            );
        }
    ...
    public function addnumcart(Request $request)
    {
        ...
        $result = $cart->addnum($prod, $prod->id, $qty, $size,$color,$size_qty,$size_price,$size_key,$keys,$values,$affilate_user);
