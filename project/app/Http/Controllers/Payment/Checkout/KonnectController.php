...
class KonnectController extends CheckoutBaseControlller
{
    private $apiKey;
    private $walletId;
    private $sandbox;
    ...
    public function store(Request $request)
    {
        ...
        try {
            $response = Http::withHeaders([
                ...
                'amount' => intval($total * 1000), // millimes
                'token' => 'TND',
                'type' => 'immediate',
                'description' => $this->gs->title . ' Order #' . $orderNumber,
                'acceptedPaymentMethods' => ['wallet', 'bank_card', 'e-DINAR'],
                'lifespan' => 20, // minutes
                'checkoutForm' => true,
                'addPaymentFeesToAmount' => false,
                'orderId' => $orderNumber,
                'successUrl' => route('front.konnect.success'),
                'failUrl' => route('front.konnect.cancel'),
                'webhook' => route('front.konnect.notify'),
            ]);

            $result = $response->json();

            if ($response->successful() && isset($result['payUrl'])) {
                Session::put('konnect_input', $input);
                Session::put('konnect_order_number', $orderNumber);
                Session::put('konnect_amount', $total);
                Session::put('konnect_payment_ref', $result['paymentRef'] ?? null);

                return redirect()->away($result['payUrl']);
            }

            $errorMsg = $result['message'] ?? 'Konnect payment initiation failed.';
    ...
    public function success(Request $request)
    {
        ...
        if (!$paymentRef) {
            return redirect()->route('front.payment.cancle')->with('unsuccess', __('Payment reference missing.'));
        }
        ...
        try {
            ...
            if ($response->successful() && isset($result['payment']['status']) && $result['payment']['status'] === 'completed') {
                $input = Session::get('konnect_input');
                $orderNumber = Session::get('konnect_order_number');
                $total = Session::get('konnect_amount');

                return $this->createOrder($input, $orderNumber, $total, $paymentRef);
            }
    ...
    {
        // Konnect sends webhook after payment
        $paymentRef = $request->payment_ref;
        
        // Verify and process (similar to success but return JSON)
        return response()->json(['status' => 'received']);
    }

    public function cancel()
    {
        Session::forget('konnect_input');
        Session::forget('konnect_order_number');
        Session::forget('konnect_amount');
    ...
    private function createOrder($input, $orderNumber, $total, $txnId)
    {
        ...
        $data = [
            ...
            'body' => "Hello Admin!<br>Your store has received a new order via Konnect.<br>Order Number is ".$order->order_number.".<br>Thank you.",
        ];
        $mailer = new XMerchMailer();
