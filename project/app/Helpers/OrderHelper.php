...
class OrderHelper
{
    ...
    /**
     * POD: Create print jobs for POD products in the order
     */
    public static function create_print_jobs($cart, $order){
        try{
            if (!class_exists('App\\Models\\PrintJob')) {
                // PrintJob model doesn't exist yet, skip
                return;
            }
            ...
            foreach($cart->items as $prod)
            {
                ...
                if($product && isset($product->is_pod) && $product->is_pod == 1)
                {
                    ...
                    $printJob->estimated_time_minutes = ($product->print_time_minutes ?? 30) * $prod['qty'];
            ...
            \Log::error("Error creating print jobs: " . $e->getMessage());
            return 0;
        }
    }

    /**
     * POD: Check if order has POD products
     */
    public static function has_pod_products($cart){
        try{
            foreach($cart->items as $prod)
            {
                $product = Product::find($prod['item']['id']);
    ...
    /**
     * POD: Skip stock reduction for POD products
     */
    public static function stock_check_pod_aware($cart){
        try{
            foreach($cart->items as $prod)
            {
                $product = Product::find($prod['item']['id']);
                
                // Skip stock reduction for POD products
                if($product && isset($product->is_pod) && $product->is_pod == 1)
                {
                    continue;
                }
