...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$color, $keys, $values) {
        // POD CAPACITY CHECK
        if (isset($item->is_pod) && $item->is_pod) {
            if (!$this->checkPodCapacity($item, 1)) {
                // Set error flag that can be checked by controller
                $this->pod_capacity_error = true;
                $this->pod_remaining = $item->remaining_capacity ?? 0;
                return false;
            }
        }
        
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values,'item_price' => $item->price,'discount' => 0,'affilate_user' => 0, 'is_pod' => $item->is_pod ?? 0];
...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key, $keys, $values, $affilate_user) {
        // POD CAPACITY CHECK
        if (isset($item->is_pod) && $item->is_pod) {
            if (!$this->checkPodCapacity($item, $qty)) {
                // Set error flag that can be checked by controller
                $this->pod_capacity_error = true;
                $this->pod_remaining = $item->remaining_capacity ?? 0;
                return false;
...
// **************** REDUCING QUANTITY ENDS *******************
    ...
    // ==================== POD CAPACITY CHECKING ====================

    /**
     * Check if POD product has enough capacity for the requested quantity
     */
    protected function checkPodCapacity($item, $quantity) {
        if (!isset($item->is_pod) || !$item->is_pod) {
            return true; // Not a POD product, no capacity check needed
        }

        // Check if Product model has the isAvailable method
        if (method_exists($item, 'isAvailable')) {
            return $item->isAvailable($quantity);
        }

        // Fallback: check remaining_capacity attribute
        if (isset($item->remaining_capacity)) {
            return $item->remaining_capacity >= $quantity;
        }

        // If no capacity checking available, allow it
        return true;
    }

    /**
     * Get total POD items quantity in cart for a specific product
     */
    protected function getPodQuantityInCart($productId) {
        $total = 0;
        if ($this->items) {
            foreach ($this->items as $item) {
                if (isset($item['item']) && $item['item']->id == $productId && ($item['is_pod'] ?? false)) {
                    $total += $item['qty'];
                }
            }
        }
